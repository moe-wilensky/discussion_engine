...
class TestTerminationConditions:
    ...
    def test_archive_when_one_or_fewer_responses(self, setup_termination_scenario):
        ...
        Response.objects.create(
            round=round, user=data['users'][0],
            content='Only response', character_count=13
        )
    ...
    def test_archive_when_max_responses_reached(self, setup_termination_scenario):
        ...
        current_round = Round.objects.get(
            discussion=discussion, round_number=3
        )
        Response.objects.create(
            round=current_round, user=data['users'][0],
            content='Extra', character_count=5
        )
    ...
    def test_archive_when_all_participants_permanent_observers(self, setup_termination_scenario):
    ...
    def test_archival_process(self, setup_termination_scenario):
        ...
        for user in data['users']:
            Response.objects.create(
                round=round, user=user,
                content='Test', character_count=4
            )
        
        MultiRoundService.archive_discussion(
            discussion, "Test archival reason"
        )
    ...
    def test_termination_conditions_checked_in_order(self, setup_termination_scenario):
        ...
        Response.objects.create(
            round=round, user=data['users'][0],
            content='Only one', character_count=8
        )
