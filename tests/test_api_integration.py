...
class TestCompleteRegistrationFlow:
    ...
    def test_full_registration_with_invite(self, api_client, user_factory, discussion_factory, response_factory):
...
class TestCompleteInviteFlow:
    ...
    def test_full_discussion_invite_flow(
        ...
        with patch('core.tasks.send_invite_notification.delay'):
            response = authenticated_client.post('/api/invites/discussion/send/', {
        ...
        invitee_client.credentials(HTTP_AUTHORIZATION=f'Bearer {str(refresh.access_token)}')
...
class TestCompleteJoinRequestFlow:
    ...
    def test_full_join_request_flow(
        ...
        requester_client.credentials(HTTP_AUTHORIZATION=f'Bearer {str(refresh.access_token)}')
        ...
        approver_client.credentials(HTTP_AUTHORIZATION=f'Bearer {str(refresh.access_token)}')
        
        response = approver_client.get(f'/api/discussions/{discussion.id}/join-requests/')
        assert response.status_code == 200
        assert len(response.data['pending']) == 1
