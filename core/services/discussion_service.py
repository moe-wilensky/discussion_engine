...
from core.models import User, Discussion, DiscussionParticipant, PlatformConfig, Round
...
class DiscussionService:
    ...
    def create_discussion(
        ...
        # Validate parameters
        is_valid, error_msg = DiscussionPreset.validate_parameters(
            mrm, rtm, mrl, config
        )
        if not is_valid:
        ...
        if not config.allow_duplicate_discussions:
            if DiscussionService.check_duplicate(headline, config):
                raise ValidationError("A discussion with this headline already exists")
            ...
            for invitee in initial_invites:
                try:
                    InviteService.send_discussion_invite(
                        inviter=initiator, invitee=invitee, discussion=discussion
                    )
    ...
    def get_observable_discussions(user: User) -> QuerySet:
        ...
        """
        return (
            Discussion.objects.filter(participants__user=user, status="active")
            .distinct()
            .select_related("initiator")
            .prefetch_related("participants__user")
        )
    ...
    def check_duplicate(headline: str, config: PlatformConfig) -> bool:
        ...
        return Discussion.objects.filter(
            topic_headline__iexact=headline, status="active"
        ).exists()
    ...
    def get_discussion_status(discussion: Discussion, user: User) -> dict:
        ...
        # Get current round
        current_round = (
            discussion.rounds.filter(status="in_progress")
