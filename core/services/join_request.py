...
class JoinRequestService:
    ...
    @staticmethod
    def create_request(
        discussion: Discussion, requester: User, message: str = ""
    ) -> JoinRequest:
        """
        ...
        if DiscussionParticipant.objects.filter(
            discussion=discussion, user=requester
        ).exists():
        ...
        if JoinRequest.objects.filter(
            discussion=discussion, requester=requester, status="pending"
        ).exists():
            raise ValidationError(
                "You already have a pending request for this discussion"
            )
    ...
    def approve_request(request: JoinRequest, approver: User) -> DiscussionParticipant:
        ...
        with transaction.atomic():
            ...
            participant = DiscussionParticipant.objects.create(
                discussion=request.discussion, user=request.requester, role="active"
            )
