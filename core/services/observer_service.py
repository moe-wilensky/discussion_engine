...
class ObserverService:
    ...
    def move_to_observer(
        participant: DiscussionParticipant, reason: str, posted_in_round: bool = False
    ) -> None:
        """
        ...
        with transaction.atomic():
            ...
            NotificationService.send_moved_to_observer_notification(
                participant.user, participant.discussion, "temporary_observer", reason
            )

    @staticmethod
    def can_rejoin(
        participant: DiscussionParticipant, current_round: Round
    ) -> Tuple[bool, str]:
        """
        ...
        if not removal_round:
            ...
            removal_round = (
                Round.objects.filter(discussion=participant.discussion)
        ...
        if (
            ...
            if current_round.round_number == removal_round.round_number:
                ...
                if removal_round.final_mrp_minutes:
                    elapsed_minutes = (
                        timezone.now() - participant.observer_since
                    ).total_seconds() / 60
                    if elapsed_minutes >= removal_round.final_mrp_minutes:
                        ...
                        return (
                            False,
                            f"wait_{removal_round.final_mrp_minutes - elapsed_minutes:.1f}_minutes",
                        )
                else:
        ...
        if participant.observer_reason == "mrp_expired":
            ...
            if current_round.round_number == next_round_number:
                ...
                if current_round.final_mrp_minutes:
                    ...
                    elapsed_in_round = (
                        timezone.now() - current_round.start_time
                    ).total_seconds() / 60
                    if elapsed_in_round >= current_round.final_mrp_minutes:
                        ...
                        return (
                            False,
                            f"wait_{current_round.final_mrp_minutes - elapsed_in_round:.1f}_minutes_in_round_{next_round_number}",
                        )
                else:
    ...
    def get_wait_period_end(
        ...
        if (
            ...
            return participant.observer_since + timedelta(
                minutes=removal_round.final_mrp_minutes
            )
        ...
        if (
            ...
            if next_round and next_round.final_mrp_minutes:
                return next_round.start_time + timedelta(
                    minutes=next_round.final_mrp_minutes
                )
            else:
    ...
    def rejoin_as_active(participant: DiscussionParticipant) -> None:
        ...
        current_round = (
            Round.objects.filter(
                discussion=participant.discussion, status="in_progress"
            )
            .order_by("-round_number")
            .first()
        )
    ...
    def make_permanent_observer(
        participant: DiscussionParticipant, reason: str
    ) -> None:
        """
