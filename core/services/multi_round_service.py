...
class MultiRoundService:
    ...
    def check_termination_conditions(
        discussion: Discussion, round: Round, config: PlatformConfig
    ) -> Tuple[bool, str]:
        """
        ...
        if response_count <= 1:
            return (
                True,
                f"Round {round.round_number} received only {response_count} response(s)",
            )
        ...
        if config.max_discussion_responses > 0:
            ...
            if total_responses >= config.max_discussion_responses:
                return (
                    True,
                    f"Reached maximum responses of {config.max_discussion_responses}",
                )
    ...
    def archive_discussion(discussion: Discussion, reason: str) -> None:
        ...
        with transaction.atomic():
            ...
            # Lock all responses
            Response.objects.filter(round__discussion=discussion).update(is_locked=True)
