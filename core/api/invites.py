...
def my_invites(request):
    ...
    can_send_platform = user.platform_invites_banked > 0 and total_responses >= config.responses_to_unlock_invites
    can_send_discussion = user.discussion_invites_banked > 0 and total_responses >= config.responses_to_unlock_invites
...
def send_discussion_invite(request):
    ...
    try:
        ...
        return Response(
            {'error': 'Discussion or user not found'},
            status=status.HTTP_404_NOT_FOUND
        )
    
    try:
        invite = InviteService.send_discussion_invite(
            request.user,
            discussion,
            invitee
        )
...
def accept_invite(request, invite_id):
    ...
    try:
        ...
        return Response({
            ...
            'discussion_id': str(invite.discussion.id) if invite.discussion else None
        }, status=status.HTTP_200_OK)
