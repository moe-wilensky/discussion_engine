...
def voting_status(request, discussion_id, round_number):
    ...
    return APIResponse({
        ...
        'window_closes_at': window_closes_at.isoformat() if window_closes_at else None,
...
def parameter_results(request, discussion_id, round_number):
    ...
    mrl_will_pass = (mrl_counts[mrl_winner] + (mrl_counts['not_voted'] if mrl_winner == 'no_change' else 0)) >= mrl_counts['majority_needed']
    rtm_will_pass = (rtm_counts[rtm_winner] + (rtm_counts['not_voted'] if rtm_winner == 'no_change' else 0)) >= rtm_counts['majority_needed']
...
def observer_status(request, discussion_id):
    ...
    if current_round:
        ...
        can_rejoin_at = ObserverService.get_wait_period_end(
            participant, current_round
        )
    ...
    return APIResponse({
        ...
        'observer_since': participant.observer_since.isoformat() if participant.observer_since else None,
